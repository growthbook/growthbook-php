...
class Condition {
  ...
  private static function evalOr(array $attributes, array $conditions): bool {
    ...
    foreach($conditions as $condition) {
      if(static::evalCondition($attributes, $condition)) return true;
    }
    return false;
  }
  ...
  private static function getPath(array $attributes, string $path): mixed {
    ...
    foreach($parts as $part) {
      if(!array_key_exists($part, $current)) return null;
  ...
  private static function elemMatch(array $condition, mixed $attributeValue): bool {
    ...
    foreach($attributeValue as $item) {
      if(static::isOperatorObject($condition)) {
        if(static::evalConditionValue($condition, $item)) return true;
      }
      elseif(static::evalCondition($item, $condition)) return true;
    }
    return false;
  }
  ...
  private static function evalOperatorCondition(string $operator, mixed $attributeValue, mixed $conditionValue): bool {
    switch($operator) {
      ...
      case '$in':
        if(!is_array($conditionValue)) return false;
      ...
      case '$all':
        if(!is_array($attributeValue) || !is_array($conditionValue)) return false;
        ...
        return true;
      case '$exists':
        if(!$conditionValue) return $attributeValue === null;
